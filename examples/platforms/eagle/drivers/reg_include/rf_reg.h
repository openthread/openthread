...
#define REG_SC_BASE_ADDR			0x1401c0

#define reg_busclk_ratio		REG_ADDR8(REG_SC_BASE_ADDR+0x18)

#define reg_clksel0				REG_ADDR8(REG_SC_BASE_ADDR+0x28)
enum
{
	FLD_SCLK_DIV					= BIT_RNG(0,3),
	FLD_SCLK_SEL					= BIT_RNG(4,6),
	FLD_MSPI_CLK_SEL				= BIT(7),

};

#define BB_LL_BASE_ADDR 	0x140a00
#define reg_ll_irq_list_l  	REG_ADDR16(BB_LL_BASE_ADDR+0x20)
enum
{
	FLD_ZB_RX_IRQ         = BIT(0),
	FLD_ZB_TX_IRQ         = BIT(1),
	FLD_RX_TIMEOUT_IRQ 	  = BIT(2),
	FLD_FIFO_FULL_IRQ     = BIT(3),
	FLD_CRC2_IRQ          = BIT(4),
	FLD_CMD_DONE_IRQ      = BIT(5),
};
#define reg_bb_auto_ctrl				REG_ADDR8(0x10050c)

enum{
	TX_MULTI_EN					=	BIT(0),
	RX_MULTI_EN					=	BIT(1),
	CH_0_RNUM_EN_BK				=	BIT(2),
	CH_1_RNUM_EN_BK				=	BIT(3),
	CH1_RX_ERR_EN				=	BIT(4),
	DMA_REQ_D1_EN				=	BIT(6),
};
#define reg_bb_tx_chn_dep				REG_ADDR8(0x1004f3)
#define reg_bb_tx_size					REG_ADDR8(0x1004f2)

#define reg_bb_rx_size						REG_ADDR8(CHNADDR+0xf6)

#define reg_dmac_ch0_cr0					REG_ADDR8(CHNADDR+0x44)
enum
{
	FLD_ENABLE					= BIT(0),
	FLD_TC_MASK					= BIT(1),
	FLD_ERR_MASK				= BIT(2),
	FLD_ABT_MASK				= BIT(3),
	FLD_REQ_SEL					= BIT_RNG(4,7),
};

#define reg_rx_wptr_mask					REG_ADDR8(CHNADDR+0x10d)
...
#define    reg_rf_ll_ctrl1          REG_ADDR8(REG_BB_LL_BASE_ADDR+0x03)
enum{
	FLD_RF_FSM_TIMEOUT_EN        =	BIT(0),
	FLD_RF_R_RX_FST_TIMEOUT_EN   =	BIT(1),
	FLD_RF_R_RX_TIMEOUT_EN       =	BIT(2),
	FLD_RF_R_CRC2_EN             =	BIT(3),
	FLD_RF_R_INIT_PRX_PID        =	BIT_RNG(4,5),
	FLD_RF_R_INIT_PTX_PID        =	BIT_RNG(6,7),
};
...
enum{
	FLD_RF_ZB_RX_IRQ              =	BIT(0),
	FLD_RF_ZB_TX_IRQ              =	BIT(1),
	FLD_RF_RX_TIMEOUT_IRQ         =	BIT(2),
	FLD_RF_RX_FIFO_FULL_IRQ       =	BIT(3),
	FLD_RF_RX_CRC2_IRQ            =	BIT(4),
	FLD_RF_CMD_DONE_IRQ           =	BIT(5),
	FLD_RF_FSM_TIMEOUT_IRQ        =	BIT(6),
	FLD_RF_TX_RETRYCNT_IRQ        =	BIT(7),

	FLD_RF_TX_DS_IRQ              =	BIT(8),
	FLD_RF_RX_DR_IRQ              =	BIT(9),
	FLD_RF_RX_FST_TIMEOUT_IRQ     =	BIT(10),
	FLD_RF_RX_INVLD_PID_IRQ       =	BIT(11),
	FLD_RF_S_TX_TIMEOUT_IRQ       =	BIT(12),
	FLD_RF_WIFI_DENY_IRQ          =	BIT(13),
	FLD_RF_SUPP_OF_IRQ            =	BIT(14),
	FLD_RF_RXDMA_OF_IRQ           =	BIT(15),
};


#define    reg_rf_ll_irq_list_h     REG_ADDR8(REG_BB_LL_BASE_ADDR+0x21)
