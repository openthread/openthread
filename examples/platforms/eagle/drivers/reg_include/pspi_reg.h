 ...
 * BIT[4:7]   the minimum time that SPI CS should stay HIGH.the actual duration is (SPI_CLK period_out /
 * 2)*(csht+1).default=2,master only
 */
...
enum
{
    ...
    FLD_PSPI_CSHT            = BIT_RNG(4, 7), // default=2,the minimum time that SPI CS should stay HIGH.the actual duration is
                                   // (SPI_CLK period / 2)*(csht+1)
};
