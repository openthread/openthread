...
/*
 * This register represents Signal frame polarity of PWM5~PWM0.By default, PWM outputs high level under Count status and
 * low level under Remaining status. If the corresponding bit is set to 1, the high and low levels in different states
 * will be reversed. BIT(0):pwm0 out low level first. ~BIT(0):pwm0 out high level first. BIT(1):pwm1 out low level
 * first. ~BIT(1):pwm1 out high level first. BIT(2):pwm2 out low level first. ~BIT(2):pwm2 out high level first.
 ...
 * BIT[0]:If this bit is set, an interrupt will be generated after a set of pulses has been sent. When this interrupt is
 * enabled, you can capture an interrupt after a pulse is sent by detecting whether bit[0] of 0x140431 is set.
 ...
 * BIT[16]:The Bit is to enable the mask_lvl(This level specifically indicates the number of bytes in the FIFO that can
 * trigger an interrupt) interrupt.
 */
 ...
 * BIT[0]:This bit is usually used with BIT[0] of 0x140430. If this bit is set to 1, it means that a pulse group of PWM
 * has been sent.An interrupt was also generated. You can manually write 1 to clear the interrupt flag. BIT[1]:In ir dma
 * fifo mode. If this is set 1 Indicates that a set of pulse of group has been sent. You can manually write 1 to clear
 * the interrupt flag. BIT[2]:If this is set 1 Indicates that a signal frame interrupt has been generated. You can
 * manually write 1 to clear the interrupt flag. BIT[3]:If this is set 1 Indicates that a signal frame interrupt has
 * been generated. You can manually write 1 to clear the interrupt flag. BIT[4]:If this is set 1 Indicates that a signal
 * frame interrupt has been generated. You can manually write 1 to clear the interrupt flag. BIT[5]:If this is set 1
 * Indicates that a signal frame interrupt has been generated. You can manually write 1 to clear the interrupt flag.
 * BIT[6]:If this is set 1 Indicates that a signal frame interrupt has been generated. You can manually write 1 to clear
 * the interrupt flag. BIT[7]:If this is set 1 Indicates that a signal frame interrupt has been generated. You can
 * manually write 1 to clear the interrupt flag. BIT[16]:When the FIFO value is less than the set value, an interrupt is
 * generated(The premise is that this interrupt has been enabled by register 0x140432 previous). The user can know
 * whether this interrupt is generated by reading the status of this register.If BIT(16):1 Indicates that this interrupt
 * has been generated.
 */
...
/**
 * [7:0] bits 7-0 of PWM0's high time or low time(if pola[0]=1),if shadow bit(fifo data[14]) is 1 in ir fifo mode or dma
 * fifo mode.
 */
#define reg_pwm_tcmp0_shadow REG_ADDR16(REG_PWM_BASE + 0x44)

/**
 * [15:8] bits 15-8 of PWM0's high time or low time(if pola[0]=1),if shadow bit(fifo data[14]) is 1 in ir fifo mode or
 * dma fifo mode.
 */
