 ...
 *  couple of styles and variants, all intended to provide parallelism
 *  with the run-time interfaces.
 *
 ...
 *  [assert()](http://pubs.opengroup.org/onlinepubs/009695399/functions/assert.html))
 *  and are __completely__ compiled out in production build
 *  configurations, when #NL_ASSERT_PRODUCTION is enabled.
 *
 ...
 *
 *  These are __completely__ compiled out in production build
 *  configurations, when #NL_ASSERT_PRODUCTION is enabled.
 *
 ...
 *
 * The following table indicates the availablity of the run-time interface
 ...
 *  counterparts. Further, rather than altering run-time execution on
 *  assertion firing against a dynamically-checked run-time condition,
 *  these interfacs terminate compilation against a statically-checked
 ...
 *
 *  These assertions are __completely__ compiled out in production build
 *  configurations, when #NL_ASSERT_PRODUCTION is enabled.
 *
 *  @subsubsection compile-time-abort Abort
 *
 *  These assertions are identical to the @ref compile-time-assert
 *  style assertions; however, they are active in __both__
 *  non-production __and__ production build configurations.
 *
 * @subsection compile-time-variants Variants
 *
 * The three families of run-time interface are available in one of
 * several variants, as summarized below. <em>&lt;STYLE&gt;</em> may
 * be replaced with one of (see [Compile-time Availability](@ref
 * compile-time-availability) for details):
 *
 *   * STATIC_ASSERT
 *   * STATIC_ABORT
 *
 ...
 * |:---------------------------------------------- |:------------------------------------------------------------------------------------------------------------ |
 * | nl<em>&lt;STYLE&gt;</em>                       | Base variant; execute the check.                                                                             |
 * | nl<em>&lt;STYLE&gt;</em>\_PRINT                | Execute the base check with a descriptive string. Note, this string is not  actually emitted in any meaningful way. It serves to simply comment or annotate the assertion and to provide interface parallelism with the run-time assertion interfaces.                                  |
 *
 * @section compile-time-availability Compile-time Availability
 *
 * The following table indicates the availablity of the compile-time interface
 * variants for each style.
 *
 * <table>
 *     <tr>
 ...
 * assertion interface definition (assert()), implemented atop of Nest
 ...
 *
 */
#define nlSTATIC_ABORT(aCondition)                 _nlSTATIC_ASSERT(aCondition, #aCondition)

/**
 *  @def nlSTATIC_ABORT_PRINT(aCondition, aMessage)
 *
 *  @brief
 *    This checks, at compile-time, for the specified condition, which
 *    is expected to commonly be true, and terminates compilation if
 ...
 *
 */
#define nlSTATIC_ABORT_PRINT(aCondition, aMessage) _nlSTATIC_ASSERT(aCondition, aMessage)
