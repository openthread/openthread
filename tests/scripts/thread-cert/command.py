...
def check_icmp_path(sniffer, path, nodes):
    """Verify message is forwarded along the path.
    """
    len_path = len(path)

    # Verify ICMP Request is forwarding to the next node of the path.
    for i in range(0, len_path):
        node_msg = sniffer.get_messages_sent_by(path[i])
        node_icmp_msg = node_msg.get_icmp_message(ipv6.ICMP_ECHO_REQUEST)
        ...
        if i < len_path - 1:
            ...
            temp = node_icmp_msg.icmp.header.type
        else:
            return True
